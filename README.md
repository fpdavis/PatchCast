...
The editable **Server Host Or IP** list stores a separate profile for every server used. Clicking **Connect** creates or updates that host's port, optional DPAPI-protected password, system and microphone volumes, mute choices, and pinned certificate. Selecting a saved host restores its complete profile; **Remove** deletes that host and all of its saved values. The password remains in memory for the current run even when its unlabeled save checkbox is clear. When disconnected unexpectedly, the client retries after 0, 1, 2, 4, 8, 16, and then 32 seconds, continuing every 32 seconds until **Disconnect** is selected. A connection that remains healthy for 32 seconds resets the next retry delay to zero. The Status row displays `Connected`, `Disconnected`, or `Connecting (Ns)` with a live retry countdown.
